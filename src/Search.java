    ...
    public static void main(String[] argv) {
        try {
            ...
            for(int i=0;i<ntasks;i++){
                float temp1 = (float) len * ((float) i / (float) ntasks);
                float temp2 = (float) len * (((float) i + 1) / (float) ntasks);
                //taskList.add(new SearchTask(text, pattern, len * (i / ntasks), len * ((i + 1) / ntasks)));
                if(i>0) {
                    taskList.add(new SearchTask(text, pattern, (int) temp1 - pattern.length + 1, (int) temp2));
                }else{
                    taskList.add(new SearchTask(text, pattern, (int) temp1, (int) temp2));
                }
            }
